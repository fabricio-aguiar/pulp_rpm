...
def pytest_terminal_summary(terminalreporter):
    ...
    for nodeid, (start_entries, end_entries) in grouped:
        for proccess_name, start_entry in start_entries.consumed_ram.items():
            end_entry = end_entries.consumed_ram.get(proccess_name)
            if not end_entry:
                continue
            ...
            terminalreporter.write(
                'Process: {} - LEAKED: {:.2f}MB rss | {:.2f}MB vms in {}\n'.format(
                    proccess_name, leaked_rss / 1024 / 1024, leaked_vms / 1024 / 1024, nodeid))
